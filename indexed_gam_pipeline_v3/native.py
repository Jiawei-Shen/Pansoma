...
package, compiled module included, and reports whether the tasks will use it):
    ...
    python -m <package>.native compile [--cxx g++]    (prints the build info)
    python -m <package>.native check

Portability: standard C++17 plus the pybind11 headers, no -march / fast-math flags. libstdc++
and libgcc are linked statically when the toolchain has their static archives (otherwise
dynamically), so the module needs little beyond the C library. The glibc / libstdc++ symbol
versions and CPU extensions a compiled module requires are reported by
...
from . import vg_pb2
from .candidates import Candidate, Column, Observation, Read, Visit, decode_alignment, rc
...
def compile_decoder(cxx=None, self_test_cases=5000):
    """Compile fastdecode.cpp into this package as _fastdecode<EXT_SUFFIX>, after a self-test.

    libstdc++ and libgcc are linked statically when the toolchain allows it, else dynamically.
    ...
    Returns (path, build info); no file besides the module is written.
    """
    import pybind11
    ...
    attempts = [["-static-libstdc++", "-static-libgcc"], []]
    with tempfile.TemporaryDirectory(prefix=".fastdecode-build-", dir=HERE) as temp:
        ...
        snippet = SELF_TEST_SNIPPET.format(package=__package__)
        check = subprocess.run([sys.executable, "-c", snippet, temp, str(self_test_cases)],
    ...
    info = dict(source_sha256=digest, compiler=cxx, flags=flags + extra, static_runtime=bool(extra),
                python=sys.version.split()[0], platform=sysconfig.get_platform(), pybind11=pybind11.__version__,
                self_test_records=self_test_cases)
    return output, info
...
def main(argv=None):
    ...
    if args.command == "compile":
        output, info = compile_decoder(args.cxx)
