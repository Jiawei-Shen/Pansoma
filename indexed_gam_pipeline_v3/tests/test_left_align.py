...
from .fixtures import mirror, overlap, spec_alignment  # noqa: F401
from ..candidates import Candidate, decode_alignment, indel_runs, left_align_indels, rc
...
class LeftAlignTest(unittest.TestCase):
    ...
    def test_deletion_across_nodes_is_one_candidate_with_its_path(self):
        from ..candidates import NodeReads
...
class NormalizedPlacementTest(unittest.TestCase):
    ...
    def test_an_indel_is_built_on_the_node_it_is_normalized_to(self):
        ...
        import tempfile
        from .fixtures import graph_fixture, split_args, write_gam
    ...
    def test_multi_node_deletion_build_passes_the_independent_audit(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with redirect_stdout(io.StringIO()):
                build(split_args(root, "o", gam=str(gam), graph_index=str(graph), nodes=str(root / "nodes.txt"),
                                 min_variants=1, width=11, max_indel_len=5, rows=12, debug_rows=True))
            (site,) = [json.loads(l) for l in (root / "o/INDEL/variant_summary.ndjson").read_text().splitlines()]
