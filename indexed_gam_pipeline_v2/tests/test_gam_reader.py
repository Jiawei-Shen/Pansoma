"""GAI reading/building, indexed retrieval and the CLI helpers around them."""
...
class GamReaderTest(unittest.TestCase):
    ...
    def test_foreign_tagged_groups_are_skipped(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            for nodes in ({10}, {30}, {1000}, {10, 30, 1000}):
                ...
                self.assertEqual(multiset(reader.fetch(nodes)), expected, nodes)
            self.assertEqual(multiset(IndexedGam(path, cache_bytes=0).fetch({10, 30})), multiset(rows[:3]))
    ...
    def test_discover_selects_imperfect_nodes(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            args = build_args(gam=str(path), output=str(output), min_mapq=5, node_alt=0.05,
                              chr_nodes=None, max_alignments=None, max_nodes=None)
    ...
    def test_validate_command_compares_indexed_and_sequential_retrieval(self):
        with tempfile.TemporaryDirectory() as directory:
            path, _ = tiny_gam(directory)
            folder = Path(directory)
            (folder / "nodes.txt").write_text("10\n30\n")
            args = build_args(gam=str(path), nodes=str(folder / "nodes.txt"), output=str(folder / "validation"))
            with redirect_stdout(io.StringIO()):
                validate(args)
            report = json.loads((folder / "validation" / "validation_report.json").read_text())
            self.assertTrue(report["passed"])
            self.assertEqual(report["expected_alignments"], 4)
            self.assertEqual(report["sequential_alignments"], 6)
