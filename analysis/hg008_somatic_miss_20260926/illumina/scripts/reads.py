"""Read-level view of somatic truth alleles: how do the ALT reads represent the variant (HG008-T ONT-UL, v3 tensors)?
...
For each locus (every no_candidate miss plus controls that did get a tensor):
  ...
  * decode it with the Python decoder of the FROZEN v3 source (v3_run/source; the build used the equivalent
    native decoder), restricted to the mappings around the window plus >= TRIM_BP (600) read bases on each
    side (indels are normalized across nodes; validate_trim.py checks this against whole-record decoding),
  ...
  * for ALT-like records (closer to ALT than to REF) record how the alignment represents it:
    the v3 candidates it produced there, whether one is a truth key, whether the path leaves
...
sys.path.insert(0, "/scratch/jshen/data/pansoma_v2_tensors/Liss_lab_BCM_Illumina-WGS_20240313/v3_run/source")  # the run's frozen package (indexed_gam_pipeline_v2 was removed from the repo)
from indexed_gam_pipeline_v3 import vg_pb2
from indexed_gam_pipeline_v3.gam_reader import IndexedGam
from indexed_gam_pipeline_v3.graph_index import GraphIndex
from indexed_gam_pipeline_v3.tensor_postprocessing.reference_path import ReferencePath, rc
from indexed_gam_pipeline_v3.tensor_postprocessing.truth_labels import placements

csv.field_size_limit(sys.maxsize)
RUN = Path("/scratch/jshen/data/pansoma_v2_tensors/Liss_lab_BCM_Illumina-WGS_20240313")
GAM = "/scratch/jshen/data/HG008_GIAB/vg_giraffe_GAM/Liss_lab_BCM_Illumina-WGS_20240313/HG008-T_p23_BCM_IlluminaWGS_20240313.sorted.gam"
...
TRUTH = str(Path(__file__).resolve().parent / "somatic.graph.tsv")  # snapshot of the run truth table (same under truth-labels-v1 and -v2)
OUT = Path(__file__).resolve().parent
A, SLACK, MIN_MAPQ, FAR, FETCH = 30, 10, 10, 2000, 300
TRIM_BP = int(os.environ.get("TRIM_BP", 600))  # 0 = decode whole records

sys.path.insert(0, str(RUN / "v3_run/source"))  # frozen v3 package (relative imports: import it as a package)
from indexed_gam_pipeline_v3 import candidates as dec  # noqa: E402
...
def loci():
    status = {}
    with open(OUT / "somatic.recall.v1.tsv") as f:  # snapshot; recall statuses do not depend on the label version
    ...
    controls = (rng.sample([r for r in hits if r["kind"] != "SNP" and max(len(r["ref"]), len(r["alt"])) >= 2], 150)
                ...
                + rng.sample([r for r in hits if r["kind"] == "SNP"], 50))
...
def analyse(chunk):
    ...
    with GraphIndex(GRAPH) as graph:
        for m in chunk:
            ...
            keys = set(m["keys"])
            for x in good:
                read, unsupported = dec.decode_alignment(x, sequences, 50, None)
                lin_col = {}
...
def main():
    ...
    # ONT v3 targets: the autosome node parts (the run had no supplement rounds)
    targets = NodeSet(np.concatenate([np.fromfile(f, sep=" ", dtype=np.int64)
                               for f in sorted((RUN / "v3_run/parts").glob("nodes_*.txt"))]))
