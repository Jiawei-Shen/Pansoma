...
def transitions():
    ...
    for kind in ("SNP", "DEL", "INS"):
        c = Counter((r["pacbio_status"], r["ont_status"]) for r in rows if r["kind"] == kind)
        cols = [s for s in STATUS if any(c[(a, s)] for a in STATUS)]
        print(f"\n**{kind}：PacBio v6 状态（行）→ ONT 状态（列）**\n")
        print("| PacBio \\ ONT | " + " | ".join(short[s] for s in cols) + " | 合计 |\n|---|" + "---:|" * (len(cols) + 1))
        for a in STATUS:
    ...
    c = Counter((r["kind"], r["pacbio_class"].split("_")[0], r["ont_status"]) for r in rows if r["pacbio_class"])
    classes = sorted({r["pacbio_class"].split("_")[0] for r in rows if r["pacbio_class"]}, key=lambda x: (x[0], int(x[1:].rstrip("abc") or 0), x))
    print("| PacBio 类别 | 种类 | " + " | ".join(short[s] for s in cols) + " |\n|---|---|" + "---:|" * 4)
    for kind in ("SNP", "DEL", "INS"):
        for cl in classes:
...
def miss_classes():
    ...
    v5 = list(csv.DictReader(open(PB / "indel_no_candidate.tsv"), delimiter="\t"))  # PacBio v6
    ...
    print(f"\n### ONT 没有候选的 INDEL：原因分类（ONT INS {n6['INS']:,}、DEL {n6['DEL']:,}；PacBio v6 INS {n5['INS']:,}、DEL {n5['DEL']:,}）\n")
    print("| 类别 | INS ONT | INS PacBio | DEL ONT | DEL PacBio | ALT 比例中位数 (ONT) | 按无误 ALT reads 判定 (ONT) | PASS∩BED (ONT) |")
    print("|---|---:|---:|---:|---:|---:|---:|---:|")
    for cl in classes:
    ...
    for r in tr:
        ...
        if r["kind"] == "SNP":
            continue
        grp = "有 tensor" if r["ont_status"].startswith("tensor") else "filtered" if r["ont_status"] == "filtered" else "没有候选" if r["ont_status"] == "no_candidate" else "其它"
        L[(r["kind"], grp, b(n))] += 1
...
def snv():
    ...
    c, p = Counter(r["final_class"] for r in rows), Counter(r["final_class"] for r in pb)
    print(f"\n### SNV 没有候选（ONT {len(rows)}；PacBio v6 {len(pb)}）\n")
    print("| 类别 | ONT | ONT 占比 | ONT ALT 比例中位数 | PacBio v6 |\n|---|---:|---:|---:|---:|")
    for k in sorted(set(c) | set(p)):
        fr = sorted(float(r["alt_fraction"]) for r in rows if r["final_class"] == k and r["alt_fraction"])
        print(f"| {k} | {c[k]} | {pct(c[k], len(rows))} | {fr[len(fr) // 2] if fr else '-'} | {p[k]} |")


if __name__ == "__main__":
    what = sys.argv[1:] or ["recall", "transitions", "classes", "residual", "controls", "snv"]
    if "recall" in what:
        recall_table(RUN / "v3_tensors/somatic.recall.tsv", "ONT somatic 召回")
