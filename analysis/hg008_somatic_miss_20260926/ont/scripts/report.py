"""Tables and numbers for REPORT.md from reads_part*.jsonl + snv_detail.jsonl + snv_nodes.json (HG008-T ONT-UL, v3 tensors).

Writes:
  indel_no_candidate.tsv                 every INS/DEL truth allele without an ONT candidate (one row each)
  snv_no_candidate.tsv                   every SNV truth allele without an ONT candidate (+ SNV detail columns)
  controls.tsv                           truth alleles that got an ONT tensor, analysed the same way
  residual_edits_labelled_germline.tsv   residual edits (>= 3 ALT reads) of misses whose tensor is labelled germline
  transitions.tsv                        PacBio v6 status / class -> ONT status / class for every truth allele
...
def tensor_labels():
    """{candidate_id of every allele of every labelled ONT tensor: (label name, reason, representative?)}."""
    result = {}
...
def residual(r, min_reads=3):
    """Edits the ALT reads make at the site (>= min_reads reads), with the label their ONT tensor got."""
    out = []
...
for f in ("indel_no_candidate.tsv", "snv_no_candidate.tsv"):
    for r in csv.DictReader(open(PACBIO_ANALYSIS / f), delimiter="\t"):
        pb_class[(r["chrom"], r["vcf_pos"], r["vcf_ref"], r["vcf_alt"])] = r["final_class"]
ont_class = {r["truth_id"]: r["final_class"] for r in misses}
with open(HERE / "transitions.tsv", "w") as out:
    ...
    for tid, t in truth.items():
        k = pb_class.get((t["chrom"], t["vcf_pos"], t["vcf_ref"], t["vcf_alt"]), "")
        out.write(f"{tid}\t{t['chrom']}\t{t['vcf_pos']}\t{t['kind']}\t{t['passed']}\t{t['in_bed']}\t{pb_status[tid]}\t{k}"
                  f"\t{ont_status[tid]}\t{ont_class.get(tid, '')}\n")
