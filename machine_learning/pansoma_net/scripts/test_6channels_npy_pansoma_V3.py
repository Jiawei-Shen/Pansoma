#!/usr/bin/env python3
"""
ONE-STOP pipeline (combined from your 3 scripts):

1) Run fast batch inference over .npy tensors (single or shard mode; shard mode is mmap-safe).
2) Join predictions to the LATEST shard-format variant_summary.ndjson using:
       (shard_index, index_within_shard)
3) Emit VCFs (bgzip + optional tabix index):
       --emit linear   (default) : linear-reference VCF (only records that can be anchored)
       --emit all                 : node-form VCF for ALL kept records
       --emit ref                 : node-form VCF for "reference" nodes only (requires --map_json)

Definitions:
- Kept predictions:
    * pred_class == "true"  -> PASS (optionally gated by --min_true_prob)
    * else if --refcall_prob provided and prob >= it -> FILTER=RefCall
    * else drop
- Node-form VCF semantics:
    CHROM=node_id
    POS = v_pos+1 by default (since meta v_pos is 0-based); use --pos_is_1based if not.
    INFO includes: PROB, AF, DP, AD, OTHER, BQ, TYPE, SHARD, IDX, plus NID/NSO.

Linear conversion (for --emit linear):
- Requires anchors from --map_json (node map JSON/JSONL).
- Optional --tsv provides ALT->REF node mapping (ALT node borrows REF anchor).
- Uses NSO as the original offset (0-based by default); see --offset_is_0_based.

Outputs:
- With --out_prefix PREFIX and --emit:
    all    -> PREFIX.all.vcf.gz
    ref    -> PREFIX.ref.vcf.gz
    linear -> PREFIX.linear.vcf.gz

Notes:
- pysam is required for bgzip/tabix. Use --no-index to skip tabix index creation.
- You can still write predictions JSON/CSV (optional).
"""
...
import sys
import time
from typing import Dict, List, Tuple, Optional, Any, Union, Iterator, Iterable, Set
...
def load_variant_summary_index(ndjson_path: str) -> Dict[Tuple[int, int], dict]:
    """
    Map (shard_index, index_within_shard) -> meta dict
    """
...
def load_node_map(
    ...
    Set[int],                              # nodes_with_genomead_af
]:
...
def true_prob_from_probs_or_predprob(probs_dict: Optional[dict], pred_prob: Any) -> Optional[float]:
...
def write_vcf_bgzip(
    ...
    """
    records: list of (CHROM, POS, ID, REF, ALT, QUAL, FILTER, INFO)
    """
    ensure_pysam_or_die()
    ...
    # contigs in encounter order after sorting
    contigs: List[str] = []
    seen: Set[str] = set()
...
def build_node_vcf_record_from_meta(
    ...
    """
    Node-form VCF record:
      CHROM=node_id
      POS = v_pos+1 by default (since v_pos typically 0-based)
      INFO includes SHARD/IDX etc
    """
    try:
...
def convert_node_record_to_linear(
    ...
    """
    Input node_rec: (CHROM=node_id, POS=vcf_pos, ..., INFO includes NSO=<offset>)
    We use:
      node_id from CHROM
      offset from INFO: NSO
    Then anchor lookup:
      try ref_node=alt_to_ref.get(node_id) first, else node_id
    """
            ...
            for images, shard_paths, index_in_shard, shard_index in dl:
                ...
                for i in range(bs):
                    ...
                    if hasattr(si_val, "item"):
                        si_val = si_val.item()
                    if si_val is None:
                        si_val = shard_index_from_path(sp)
                    try:
                        sidx = int(si_val) if si_val is not None else None
                    except Exception:
                        sidx = None
                    ...
                    if sidx is None or iws is None:
                        missing_meta += 1
                        continue
...
def main():
    ...
    # Linear conversion inputs (needed if emitting linear, and for ref classification if emitting ref)
