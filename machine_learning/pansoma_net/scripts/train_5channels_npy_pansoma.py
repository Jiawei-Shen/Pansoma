...
def _iter_sharded_batches(
    ...
    prefetcher = ShardPrefetcher(ordered_shards, max_prefetch=1)
...
def _iter_sharded_batches_ddp(
    ...
    prefetcher = ShardPrefetcher(local_shards, max_prefetch=1)
    ...
    if resume is not None and os.path.isfile(resume):
        try:
            ...
            if ckpt_in_channels is not None and int(ckpt_in_channels) != 5:
                raise ValueError(
                    f"Checkpoint in_channels={ckpt_in_channels}, but this script expects 5-channel tensors."
                )
