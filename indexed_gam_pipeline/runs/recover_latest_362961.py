...
PRIOR = BASE/'recovery_remaining121_p32_full_cache8g_release_20260922'
ROOT = BASE/'recovery_remaining74_p32_full_targetobs_release_20260922'
...
def prepare():
    ...
    assert len(done)==47 and len(remaining)==74
    ...
    print('Verified 495 original and 438 completed recovery tasks', flush=True)
    for name in ('build_v2.py','candidates.py'):
        assert digest(REPO/'indexed_gam_pipeline'/name) == digest(REPO/'tmp/target_release_benchmark/optimized/indexed_gam_pipeline'/name), 'Code differs from tested version'
    ...
    inventory.update(status='prepared', recovery_run=str(ROOT), recovery_nodes=count,
                     recovery_tasks=74, preserved_outputs=entries,
    ...
    verify(c)
    print(json.dumps(dict(root=str(ROOT),nodes=count,tasks=74,processes=32)),flush=True)
