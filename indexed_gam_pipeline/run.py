...
def main():
    ...
    for name in ("index", "discover", "validate", "build"):
        ...
        if name == "build":
            ...
            sub.add_argument("--snv-output", help="Separate SNV output directory; shared decoding")
            sub.add_argument("--indel-output", help="Separate INDEL output directory; shared decoding")
            sub.add_argument("--snv-min-af", type=float)
            sub.add_argument("--indel-min-af", type=float)
            sub.add_argument("--debug-rows", action="store_true", help="Include row paths and column graph coordinates")
            sub.add_argument("--decode-mode", choices=("auto", "full", "window"), default="auto",
                             help="auto: mean length of first 10 GAM reads <1000 bp uses full expansion; otherwise edit-window expansion")
            ...
            sub.add_argument("--early-af-filter", action=argparse.BooleanOptionalAction, default=True, help="Reject candidates whose ALT support bound / exact coverage (all records, before --max-node-reads) is below the AF threshold; default on")
            sub.add_argument("--candidate-unit", choices=["site", "allele"], default="site", help="site: one tensor per (node, start, SNV|INDEL) with all passing alleles in metadata; allele: legacy one tensor per allele")
            ...
            sub.add_argument("--max-node-reads", type=int, default=800, help="Per-node read cap applied after prefiltering, before support counting and tensor rows; deterministic by record digest; 0 disables")
